...
bool moveit_task_constructor_msgs__msg__solution_info__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("moveit_task_constructor_msgs.msg._solution_info.SolutionInfo", full_classname_dest, 60) == 0);
  }
  moveit_task_constructor_msgs__msg__SolutionInfo * ros_message = _ros_message;
  {  // id
  ...
  {  // stage_id
    PyObject * field = PyObject_GetAttrString(_pymsg, "stage_id");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->stage_id = PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // planner_id
  ...
  {  // markers
    ...
    if (!visualization_msgs__msg__Marker__Sequence__init(&(ros_message->markers), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create visualization_msgs__msg__Marker__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    visualization_msgs__msg__Marker * dest = ros_message->markers.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!visualization_msgs__msg__marker__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of SolutionInfo */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("moveit_task_constructor_msgs.msg._solution_info");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "SolutionInfo");
  ...
  {  // stage_id
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->stage_id);
    {
      int rc = PyObject_SetAttrString(_pymessage, "stage_id", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
  ...
  {  // markers
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = visualization_msgs__msg__marker__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
