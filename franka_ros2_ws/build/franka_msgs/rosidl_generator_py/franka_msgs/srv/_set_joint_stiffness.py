...
class SetJointStiffness_Request(metaclass=Metaclass_SetJointStiffness_Request):
    ...
    def __init__(self, **kwargs):
        ...
        if 'joint_stiffness' not in kwargs:
            ...
            self.joint_stiffness = numpy.array(kwargs.get('joint_stiffness'), dtype=numpy.float64)
            assert self.joint_stiffness.shape == (7, )
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if all(self.joint_stiffness != other.joint_stiffness):
            return False
        return True
