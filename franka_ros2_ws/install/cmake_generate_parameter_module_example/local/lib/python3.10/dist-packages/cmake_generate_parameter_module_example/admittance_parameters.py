...
import cmake_generate_parameter_module_example.custom_validation as custom_validators
...
class admittance_controller:
    ...
    class ParamListener:
        ...
        def refresh_dynamic_parameters(self):
            ...
            for value in updated_params.joints:
                updated_params.pid.add_entry(value)
                entry = updated_params.pid.get_entry(value)
                param_name = f"{self.prefix_}pid.{value}.p"
            ...
            for value in updated_params.joints:
                updated_params.pid.add_entry(value)
                entry = updated_params.pid.get_entry(value)
                param_name = f"{self.prefix_}pid.{value}.i"
                if not self.node_.has_parameter(self.prefix_ + param_name):
            ...
            for value in updated_params.joints:
                updated_params.pid.add_entry(value)
                entry = updated_params.pid.get_entry(value)
                param_name = f"{self.prefix_}pid.{value}.d"
                if not self.node_.has_parameter(self.prefix_ + param_name):
                ...
                self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
                entry.d = param.value
            for value in updated_params.dof_names:
                updated_params.gains.add_entry(value)
                entry = updated_params.gains.get_entry(value)
                param_name = f"{self.prefix_}gains.{value}.k"
                if not self.node_.has_parameter(self.prefix_ + param_name):
            ...
            for param in parameters:
                for value in updated_params.joints:
                    ...
                    if param.name == param_name:
                        ...
                        if validation_result:
                            return SetParametersResult(successful=False, reason=validation_result)
                        updated_params.pid.joints_map[value].p = param.value
                        self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))

                for value in updated_params.joints:
                    param_name = f"{self.prefix_}pid.{value}.i"
                    if param.name == param_name:
                        updated_params.pid.joints_map[value].i = param.value
                        self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))

                for value in updated_params.joints:
                    param_name = f"{self.prefix_}pid.{value}.d"
                    if param.name == param_name:
                        updated_params.pid.joints_map[value].d = param.value
                        self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))

                for value in updated_params.dof_names:
                    param_name = f"{self.prefix_}gains.{value}.k"
                    if param.name == param_name:
                        updated_params.gains.dof_names_map[value].k = param.value
                        self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
