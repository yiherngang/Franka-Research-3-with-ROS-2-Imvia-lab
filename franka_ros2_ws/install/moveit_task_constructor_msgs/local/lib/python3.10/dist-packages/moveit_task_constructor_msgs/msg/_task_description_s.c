...
bool moveit_task_constructor_msgs__msg__task_description__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("moveit_task_constructor_msgs.msg._task_description.TaskDescription", full_classname_dest, 66) == 0);
  }
  moveit_task_constructor_msgs__msg__TaskDescription * ros_message = _ros_message;
  {  // task_id
  ...
  {  // stages
    ...
    if (!moveit_task_constructor_msgs__msg__StageDescription__Sequence__init(&(ros_message->stages), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create moveit_task_constructor_msgs__msg__StageDescription__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    moveit_task_constructor_msgs__msg__StageDescription * dest = ros_message->stages.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!moveit_task_constructor_msgs__msg__stage_description__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * moveit_task_constructor_msgs__msg__task_description__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("moveit_task_constructor_msgs.msg._task_description");
  ...
  {  // stages
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = moveit_task_constructor_msgs__msg__stage_description__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
